...
def train(epsilon, max_updates, class_letter, train_folder, model_file_name):
    d = gen.Data()
    _lambda,m, x_pos, x_neg, m_p, m_n = d.find_scale(d.get_data(train_folder, class_letter))
    xp = d.scale_data(x_pos, _lambda, m_p)
    xn = d.scale_data(x_neg, _lambda, m_n)
    model = SVM(xp, xn, m_p, m_n, _lambda, epsilon, max_updates, class_letter)
    model.train()
...
train(epsilon, max_updates, class_letter, train_folder, model_file)